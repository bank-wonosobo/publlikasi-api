...
func NewOffice(officeService services.OfficeService,
	validator validator.Validator) *OfficeHandler {
...
func (h *OfficeHandler) Create(c *fiber.Ctx) error {
	var request dto.OfficeCreateReq
	...
	// call service
	result, err := h.officeService.Create(c.Context(), &request, file)
	if err != nil {
...
func (h *OfficeHandler) Update(c *fiber.Ctx) error {
	var request dto.OfficeUpdateReq
	id := c.Params("id")
	// parse request
	if err := c.BodyParser(&request); err != nil {
		return c.Status(fiber.StatusBadRequest).JSON(dto.CreateReponseError(err.Error()))
	...
	// call service
	result, err := h.officeService.Update(c.Context(), &request, file, id)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(dto.CreateReponseError(err.Error()))
	}
